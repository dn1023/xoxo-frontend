...
const Projects = ({ projectsData }: Props) => {
    ...
    for(let i = 0; i<projects.length; i++) {
        tempCategories.push(...projects[i].category.split(','))
    }
    const categories= [...Array.from(new Set(tempCategories))]
    ...
    const isCategoryContainProject = (pCat: string, fCat: string) => {
        const pCategories: string[] = pCat.split(',');
        let idx = pCategories.findIndex((el: string) => el.toLowerCase() == fCat.toLowerCase())
    ...
    const filterProjects = (cat: string) => {
        ...
        setFilteredProjects(projects.filter((p: project) => isCategoryContainProject(p.category, cat)));
    }
