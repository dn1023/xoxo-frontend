...
const CreateRecord = () => {
  ...
  const onRegister = async () => {
    ...
    setLoading(true);
    try {
      const response = await BotService.create(token, chatID, password); // Assuming login returns a promise
      if (response !== undefined) {
        if (response.message == "Token registered successfully!")
